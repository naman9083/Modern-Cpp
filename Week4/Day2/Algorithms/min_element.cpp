...
int main()
{
    ...
    EmployeeContainer::iterator itr = std::min_element(data1.begin(), data1.end(), [](const Employee &emp1, const Employee &emp2)
                                                       { return emp1.getSalary() < emp2.getSalary(); });

    // std::cout << (*itr).getDesignation() << "\n";
    std::cout << itr->getDesignation() << "\n";

    EmployeeRawCon::iterator itr2 = std::min_element(data2.begin(), data2.end(), [](const EmployeeRaw &emp1, const EmployeeRaw emp2)
                                                     { return emp1->getSalary() < emp2->getSalary(); });
