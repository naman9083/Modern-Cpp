...
using EngineP = std::shared_ptr<Engine>;
class Car
{
private:
	std::string carId{""};
...
public:
	...
	~Car() = default;
	Car(CarType c1,std::string carId, float price, std::string model, EngineP engine);
	Car(CarType c1,std::string carId, std::string model, EngineP engine);
